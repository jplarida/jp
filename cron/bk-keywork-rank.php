...
$rank		=		$pa_obj->getProductRankByKeyword($rank_row['keyword'],$rank_row['asin']);

echo "Keyword ID: ".$rank_row['rtk_id']."<br/>ASIN: ".$rank_row['asin']."<br/>Keyword ".$rank_row['keyword']."<br/>Rank ".$rank;


//UPDAT THE TABLES
$sql_obj->Query("UPDATE rank_tracker_keywords SET is_ranked = 1 WHERE id = '".$rank_row['rtk_id']."'");
if($rank != 0)	{
	
	//Query
	$sql_obj->Query("INSERT INTO rank_tracker_detail (rank_tracker_id, date_time, rank, keyword_id) VALUES ('".$rank_row['id']."', '$date', '$rank', '".$rank_row['rtk_id']."')");
