...
if(isset($_POST['submit'])) {
	...
	$sql_obj->Query("insert into 	
		  			products(name,asin,status,user_id,product_url)
					values('$name','$asin','$status','".$_SESSION['user_id']."','$refine_link')
					")or die(mysql_error());
			$insert_id= mysql_insert_id();	
				$t = date('Y-m-d H:i:s');
				
				/*add product in rank tracker if chackbox chacked*/
				
				if (isset($_POST['r-tracker'])) {
				$sql_obj->Query("insert into 	
		  			rank_tracker(user_id, date_time, product_id)
					values('".$_SESSION['user_id']."', '$t', '$insert_id')
					");	
				
				}
				/*add product in Super URL if chackbox chacked*/
					if (isset($_POST['s-url'])) {
						
						$sql_obj->Query("insert into 	
		  			super_url(user_id, date_time, product_id)
					values('".$_SESSION['user_id']."', '$t', '$insert_id')
					");	
						
					}
				
				/*add product in Saller Rank if chackbox chacked*/
				if (isset($_POST['s-rank'])) {
						
						$sql_obj->Query("insert into 	
		  			seller_rank(user_id, date_time, product_id)
					values('".$_SESSION['user_id']."', '$t', '$insert_id')
					");	
...
<div id="form_modal10" class="modal fade" role="dialog" aria-labelledby="myModalLabel10" aria-hidden="true">
  ...
  <tr>
    <td><input type="checkbox" name="s-url" >Super URL</td>
    <td><input type="checkbox" name="r-tracker" >Rank Tracker</td>
  </tr>
   <tr>
    <td><input type="checkbox" name="s-rank">Seller Rank</td>
    <td></td>
  </tr>
</table>
