    ...
    */
    ...
    protected function _toQueryParameterArray($prefix) {
        ...
        foreach($this->_fields as $fieldName => $fieldAttrs) {
			//echo $fieldName; print_r($fieldAttrs);
			
            $fieldType = $fieldAttrs['FieldType'];
            $fieldValue = $fieldAttrs['FieldValue'];
            $newPrefix = $prefix . $fieldName . '.';
			
			//echo $prefix." -prefix".$fieldType." fieldtype".$fieldName." field name".$newPrefix." new prefix <br />";
    ...
    private function __toQueryParameterArray($prefix, $fieldType, $fieldValue, $fieldAttrs) {
        ...
        if(is_array($fieldType)) {
            if(isset($fieldAttrs['ListMemberName'])) {
				//echo "ok fieldattr". $fieldAttrs['ListMemberName'];
                $listMemberName = $fieldAttrs['ListMemberName'];
                $itemPrefix = $prefix . $listMemberName . '.';
            } else {
				
                $itemPrefix = $prefix;
				//echo "else itmeprefix ".$indexedPrefix;
            }
			...
			//echo "complex type <br />";
            // Struct
            if(isset($fieldValue)) {
				//echo "field value is set <br />";
				//echo "<br /> field value".$fieldValue."<br />";
            ...
            if ($fieldValue!==null && $fieldValue!=="") {
				//echo "field value not null <br />";
                if ($fieldType=='bool') {
					//echo "type boolian <br />";
    ...
    protected function _toXMLFragment() 
    {
        ...
        foreach ($this->_fields as $fieldName => $field) {
			...
			//echo "fielname: ".$fieldName. " "."value: ".$field." "."field type: ".$field['FieldType']."<br />";
            if (!is_null($fieldValue) && $field['FieldType'] != "MarketplaceWebServiceProducts_Model_ResponseHeaderMetadata") {
                $fieldType = $field['FieldType'];
                if (is_array($fieldType)) {
                    if ($fieldType[0] == "object") {
						//echo "field type is object on zero index <br/>";
                        foreach ($fieldValue as $item) {
                            ...
                            $xml .= trim($xmlStr);
							//echo "after trim field value and save xml we have: ".$xml." <br/>";
                        }
                    } else if ($this->_isComplexType($fieldType[0])) {
						//echo "field type is complex <br/>";
                        if (isset($field['ListMemberName'])) {
							//echo "listmemer name is set<br/>";
                            $memberName = $field['ListMemberName'];
                            $xml .= "<$fieldName>";
                            foreach ($fieldValue as $item) {
                                ...
                                $xml .= "</$memberName>";
								//echo "after list member name and call to format xml functio we have ".$xml." <br/>";
                            }
                            $xml .= "</$fieldName>";
                        } else {
                            foreach ($fieldValue as $item) {
								//echo "not field list member value set<br/>";
                                $xml .= "<$fieldName";
                                $xml .= $item->_getAttributes();
                                $xml .= ">";
                                $xml .= $item->_toXMLFragment();
                                $xml .= "<$fieldName>";
								//echo "after get attribute function and set value we have: ".$xml." <br/>";
                            }
                        }
                    } else {
						//echo "not complex <br/>";
                        if(isset($field['ListMemberName'])) {
							//echo "not complex but list member name was set <br/>";
                            $memberName = $field['ListMemberName'];
                            $xml .= "<$fieldName>";
                            foreach ($fieldValue as $item) {
                                $xml .= "<$memberName>";
                            ...
                            $xml .= "</$fieldName>";
							//echo "after foreach not complex and member name set escape xml we have: ".$xml." <br/>";
                        } else {
                            foreach ($fieldValue as $item) {
                                $xml .= "<$fieldName>";
                                $xml .= $this->_escapeXML($item);
							...
							//echo "after foreach not complex and member name was not set escape xml we have: ".$xml." <br/>";
                        }
                    }
                } else {
					//echo "field tye is not array<br/>";
                    if ($this->_isComplexType($fieldType)) {
						//echo "field tye is not array and complex type<br/>";
                        $xml .= "<$fieldName";
						...
						//echo "field tye is not @ on 0 index<br/>";
                        $xml .= "<$fieldName>";
    ...
    protected function _getAttributes() {
		//echo "get attribute func<br />";
        $xml = "";
		//echo "this -fields: "; print_r($this->_fields); echo "<br />";
        foreach ($this->_fields as $fieldName => $field) {
            $fieldValue = $field['FieldValue'];
			//echo "in attr functio we have: fielvalue: ".$fieldValue."<br />";
            if (!is_null($fieldValue)) {
				
                $fieldType = $field['FieldType'];
				//echo "field value not null so fieltype: ".$fieldType. "<br />";
                if($fieldType[0] == "@") {
                    $xml .= " " . $fieldName . "='" . $this->_escapeXML($fieldValue) . "'";
					//echo "field type on 0 index is @ then xmla after escap xml fucn: ".$xml. "<br />";
                }
            }
        }
		//die('ok');
