...
class Reporting	{
	...
	public function __construct($am_credentials)	{
		...
		$this->aws_api_key		=	$am_credentials['aws_access_key'];
		$this->aws_secret_key	=	$am_credentials['aws_secret_key'];
		$this->marchent_id		=	$am_credentials['marketplace_seller_id'];
		$this->marketplace_id	=	$am_credentials['marketplace_id'];
		try	{
			$config = array (
			...
			$r		=	$this->service = new MarketplaceWebService_Client(
							 $am_credentials['aws_access_key'], 
							 $am_credentials['aws_secret_key'], 
							 $config,
							 APPLICATION_NAME,
							 APPLICATION_VERSION);
		}catch(Exception $e)	{
	...
	public function credentialChecker($report_type) {
		...
		$response	=	array();
		try {
              $data = $service->getReportList($request);
              if ($data->isSetGetReportListResult()) {
				  $response['msg']			=	"Your AWS credentials are verified";
				  $response['flag']			=	1;
			  }
			  
		} catch (MarketplaceWebService_Exception $ex) {
			$response['msg']			=	$ex->getMessage();
			$response['status_code']	=	$ex->getStatusCode();
			$response['error_code']		=	$ex->getErrorCode();
			$response['error_type']		=	$ex->getErrorType();
			$response['flag']			=	0;
	...
	public function invokeGetReportReqList($report_type) {
		...
		try {
              $response = $service->getReportRequestList($request);
                ...
                if ($response->isSetGetReportRequestListResult()) { 
                    ...
                    {
						//$report_info['next_token']	=	$getReportRequestListResult->getNextToken();
                   ...
                   foreach ($reportRequestInfoList as $reportRequestInfo) {
                    ...
                    if ($reportRequestInfo->isSetReportRequestId()) 
                          {
							  $report_info[$i]['req_id']	=	$reportRequestInfo->getReportRequestId();
                        //      echo("                    ReportRequestId\n");
                          ...
                          {
							   $report_info[$i]['status']	=	$reportRequestInfo->getReportProcessingStatus();
                          ...
                          {
							  $report_info[$i]['report_id']	=	 $reportRequestInfo->getGeneratedReportId();
...
/*                              echo("                    ReportProcessingStatus\n");
	...
	public function saveTheOrdersData($report_id,$user_id)	{
		...
		//for($i=29; $i< 31; $i++){
			...
			//}
				...
				echo "ORDER ALREADY EXIST, ORDER ID: ".$array['0']."<br/>";
			}
