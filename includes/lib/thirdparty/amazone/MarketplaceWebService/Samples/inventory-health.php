...
$user_row		=	$sql_obj->QFetchArray("SELECT * FROM users WHERE inventory_health = 0 ORDER BY id DESC LIMIT 1");
if(is_array($user_row))	{
	...
	if(is_array($report_row))	{
		if($report_row['report_id'] == '')	{
			...
			//UPDATE THE DATABASE
			$report_obj->updateHealthInventory($report_row['report_id']);
			
			//DELETE REPORT FOR NEW GENRATE
			$sql_obj->Query("DELETE FROM reports WHERE report_type = '$report_type'");
			
			//UPDATE USER FOR NEXT USER
			$sql_obj->Query("UPDATE users SET inventory_health = 1 WHERE id = '".$user_row['id']."'");
	...
	$sql_obj->Query("UPDATE users SET inventory_health = 0");
