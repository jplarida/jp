    ...
    <div class="row">
      <div class="col-md-12">
		...
		if(isset($not)){ getErrorMessage($not); } 
		 ?>
        <div class="col-md-12"> <a style="margin-bottom:10px;"  class="btn green" href="#form_modal10" data-toggle="modal"> <i class="fa fa-plus"></i>Add Canonical URL</a>
		 ...
		 $query 		=	mysql_query("SELECT * from canonical_url where user_id = '".$_SESSION['user_id']."'");		
  ...
  while($row		=	mysql_fetch_array($query))	{
	  ...
	  ?>
                        <tr class="odd" id="s<?php echo $row['id']; ?>">
                          ...
                          <td><a href="?cmd=del&amp;id=<?php echo $row['id']; ?>" class="btn mini red">Delete</a></td>
                        </tr>
                        <?php }?>
                      </tbody>
                      <?php }else {
