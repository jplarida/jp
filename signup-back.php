...
if(isset($_POST["submit"]))		{
	 ...
	 if ($_POST['password'] == $_POST['rpassword'])	{
		 $count	=		$sql_obj->RowCount("users","WHERE email = '$email'");
		 if($count > 0)	{
			$error	=	"This email already exist";
		 }else {
			$files	=		ImageUplloadResize("files",200,200,'user-images/');
			$sql_obj->Query("INSERT INTO 
				 users(name,email,country,user_name,password,image)
				 VALUES
				 ('$fullname',
				 '$email','$country','$username','$password','$files')");
